...
export function Footer({ lang }: FooterProps) {
  ...
  const content = {
    ko: {
      ...
      bottom: {
        copyright: "© 2025 미소♡찌개 All rights reserved",
        businessNumber: "사업자등록번호: 123-45-67890",
        telecomNumber: "통신판매업신고번호: 2024-서울강남-12345",
        address: "한국: 서울특별시 강남구 테헤란로 123 | 일본: 東京都渋谷区道玄坂1-2-3",
      },
    },
    jp: {
      brandName: "ミソ",
      ...
      bottom: {
        copyright: "© 2025 ミソ♡チゲ All rights reserved",
        businessNumber: "事業者登録番号: 123-45-67890",
        telecomNumber: "通信販売業届出番号: 2024-ソウル江南-12345",
        address: "韓国: ソウル特別市江南区テヘランロ123 | 日本: 東京都渋谷区道玄坂1-2-3",
      },
    },
  }
  ...
  return (
    <footer className="my-8 px-4 max-w-5xl text-foreground mx-auto">
      <div className="relative bg-card rounded-3xl max-w-5xl mx-auto px-4 py-10 flex flex-col md:flex-row justify-between items-center gap-6 shadow-lg border border-border">
        <div className="hidden md:block absolute -top-4 -left-8 w-[80px] h-[36px] scale-75 text-primary">
          <TapeIcon />
        </div>
        <div className="hidden md:block absolute -top-4 -right-8 rotate-90 w-[80px] h-[36px] scale-75 text-secondary">
          <TapeIcon />
        </div>
        ...
        <div className="flex flex-col md:flex-row items-start justify-between gap-4 md:gap-10 px-2 md:px-8 flex-1 w-full">
          ...
          <div className="flex flex-col gap-1 md:gap-4">
            <h4 className="uppercase font-semibold text-sm text-muted-foreground">{t.company.title}</h4>
            <div className="flex flex-col gap-2 text-sm text-foreground items-start">
              <Link className="text-muted-foreground whitespace-nowrap font-medium hover:text-primary transition-colors" href="#">
                {t.company.about}
              </Link>
              <Link className="text-muted-foreground whitespace-nowrap font-medium hover:text-primary transition-colors" href="#">
                {t.company.careers}
          ...
          {/* Support Section */}
          <div className="flex flex-col gap-1 md:gap-4">
            <h4 className="uppercase font-semibold text-sm text-muted-foreground">{t.support.title}</h4>
            <div className="flex flex-col gap-2 text-sm text-foreground items-start">
              <Link className="text-muted-foreground whitespace-nowrap font-medium hover:text-primary transition-colors" href="#">
                {t.support.terms}
              </Link>
          ...
          <div className="flex flex-col gap-1 md:gap-4">
            <h4 className="uppercase font-semibold text-sm text-muted-foreground">{t.download.title}</h4>
            <div className="flex flex-col gap-2 text-sm">
              <Button variant="outline" className="w-full justify-start bg-transparent" size="sm">
                <img src="/placeholder.svg?height=24&width=24" alt="App Store" className="w-5 h-5 mr-2" />
                App Store
              </Button>
              <Button variant="outline" className="w-full justify-start bg-transparent" size="sm">
                <img src="/placeholder.svg?height=24&width=24" alt="Google Play" className="w-5 h-5 mr-2" />
                Google Play
              </Button>
            </div>
            <div className="mt-2">
              <a
                href="mailto:contact@misojjigae.com"
                className="flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors"
              >
                <Mail className="w-4 h-4" />
                contact@misojjigae.com
              </a>
            </div>
          </div>
        </div>
      ...
      <div className="my-3 px-4 md:px-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-sm text-muted-foreground">
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-8 items-start sm:items-center">
          <p className="whitespace-nowrap">{t.bottom.copyright}</p>
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 text-xs">
            <span>{t.bottom.businessNumber}</span>
            <span>{t.bottom.telecomNumber}</span>
          </div>
        </div>
        <div className="text-xs text-muted-foreground">
          <p>{t.bottom.address}</p>
        </div>
      </div>
    </footer>
  )
